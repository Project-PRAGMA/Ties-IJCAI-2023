...
# one model supported
models = [{"model_id": 'euclidean', "params_family": {"radius": [0.5*k/m/1.87, k/m/1.87, 2*k/m/1.87], "dim": [1], "space": ["uniform"]}}]
...
for model in models:
    ...
    for params_tuple in params_tuples:
        ...
        for n in ns:
            ps = []
            for i in range(reps):
                e = mapel.elections.models_.generate_approval_votes(model_id=model_id, num_voters=n, num_candidates=m, params=params)
...
dir_csv = "../results_server/euclidean/csv_euclidean"
dir_png = "../results_server/euclidean/png_euclidean"
