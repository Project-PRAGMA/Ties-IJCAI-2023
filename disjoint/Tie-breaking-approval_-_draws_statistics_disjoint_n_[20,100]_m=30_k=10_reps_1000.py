...
# one model supported
models = [{"model_id": 'disjoint', "params_family": {"p": [0.5*k/m, k/m, 2*k/m], "phi": [0.25, 0.5, 0.75, 1.0]}}]
# models = [{"model_id": 'disjoint', "params_family": {"p": [0.5*k/m, k/m], "phi": [0.25, 0.5, 0.75, 1.0]}}]
...
for model in models:
    ...
    for params_tuple in params_tuples:
        ...
        if model_id == 'disjoint':
            g = int(1 / params["p"])
            print(m, k, params["p"], g)
            params["g"] = g
        for n in ns:
            for i in range(reps):
                e = mapel.elections.models_.generate_approval_votes(model_id='disjoint_resampling', num_voters=n, num_candidates=m, params=params)
...
dir_csv = "../results_server/disjoint/csv_disjoint"
dir_png = "../results_server/disjoint/png_disjoint"
os.makedirs(dir_csv, exist_ok=True)
os.makedirs(dir_png, exist_ok=True)
ns_name_to_save = [ns[0], ns[-1], ns[1]-ns[0] if len(ns) > 1 else ns[0]]
elections_df.to_csv(f"{dir_csv}/mapel_with_random_{model_id}_ns_{ns_name_to_save}_m_{m}_k_{k}_reps_{reps}.csv")
...
elections_results_df.to_csv(f"{dir_csv}/mapel_with_random_{model_id}_ns_{ns_name_to_save}_m_{m}_k_{k}_reps_{reps}_results.csv")
...
for params in unique_params:
    ...
    plt.savefig(f"{dir_png}/mapel_with_random_{model_id}_ns_{ns_name_to_save}_m_{m}_k_{k}_reps_{reps}_params_{' '.join([str(i) for i in params.items()])}_aggregated.png")
    plt.show()
    plt.close()
    # plt.show()

pd.DataFrame(stats_map).to_csv(f"{dir_csv}/mapel_with_random_{model_id}_ns_{ns_name_to_save}_m_{m}_k_{k}_reps_{reps}_stats.csv")
...
#         # plt.savefig(f"mapel_with_random_{model_id}_ns_{ns}_m_{m}_k_{k}_reps_{reps}_params_{' '.join([str(i) for i in params.items()])}_rule_{rule_id}.png")
#         # plt.show()
#
#
# # In[ ]:
